...
#Generamos una tabla con los aspectos a analizar ( calificaciones )
 base_pca <-  filter( bbdd, !is.na(calificacion_aroma) & !is.na(calificacion_apariencia) & !is.na(calificacion_paleta) & !is.na(calificacion_sabor) & !is.na(calificacion_total)) %>% select(calificacion_aroma, calificacion_apariencia, calificacion_paleta , calificacion_sabor, calificacion_total)
base_pca
...
correlaciones <- round(cor(select(base_pca, -calificacion_total)),2)
...
pca <- PCA( base_pca, scale.unit = TRUE, ncp = 3, quanti.sup = 5, graph = FALSE) 
...
fviz_pca_var(pca, col.var = "contrib",
             gradient.cols = c("#ffd0c7", "#ff7259", "#8b3626")
             )
```
